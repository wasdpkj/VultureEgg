...
boolean sendDATA(int _sendDATA,String _egg_UUID)
{
...
void setup() 
{
  ...
  rtc.setDate(5, 2, 8, 0, 14);  //day, weekday, month, century(1=1900, 0=2000), year(0-99)
  rtc.setTime(14, 14, 30);  //hr, min, sec
  ...
  //======================
  if(!bleConnect(egg_UUID[egg_num]))    //bleBegin
  {
#ifdef _DEBUG
    DEBUG.println("\n\r");
    DEBUG.print("[");
    DEBUG.print(egg_UUID[egg_num]);
    DEBUG.println("] ");
...
{
  {
    ...
    if(inChar==0xff && inCache==0xff)
    {
      ...
      else
      {
        ...
        DEBUG.println("\n\rsendDATA READY");
#endif
        if(sendDATA(type,egg_UUID[egg_num]))
        {
#ifdef _DEBUG
  ...
  if(egg_num_MAX>1)    //egg_num_MAX>1
  {
    ...
    if(millis()-millisSwitch>timeSwitch) 
    {
      ...
      if(!bleConnect(egg_UUID[egg_num]))
      {
#ifdef _DEBUG
        DEBUG.print("\n\r[");
        DEBUG.print(egg_UUID[egg_num]);
